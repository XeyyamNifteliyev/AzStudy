...
// Simple approach: for each slug, find the title block and use string replacement
let updatedCount = 0;

for (const [slug, langMap] of Object.entries(titleMap)) {
  // Find the title block for this slug
  const slugPattern = 'slug: "' + slug + '"';
  const slugIdx = content.indexOf(slugPattern);
  if (slugIdx === -1) continue;

  // Find "title: {" after the slug
  const titleIdx = content.indexOf('title: {', slugIdx);
  if (titleIdx === -1 || titleIdx > slugIdx + 500) continue;

  // Find the closing "    }," of the title block
  // It's the first "    }," after titleIdx
  let searchFrom = titleIdx;
  let closeIdx = -1;
  while (searchFrom < content.length) {
    const nextClose = content.indexOf('},', searchFrom);
    if (nextClose === -1) break;
    // Check if this is at 4-space indent (title block closing)
    const lineStart = content.lastIndexOf('\n', nextClose) + 1;
    const lineContent = content.substring(lineStart, nextClose + 2);
    if (lineContent.trim() === '},') {
      closeIdx = nextClose;
      break;
    }
  ...
  // Check which languages already exist in this title block
  const blockContent = content.substring(titleIdx, closeIdx);
  const existingLangs = allLangs.filter(l => blockContent.includes(l + ':'));
  const missing = allLangs.filter(l => !existingLangs.includes(l));

  if (missing.length === 0) continue;

  // Find the last language line in the block and add comma if missing
  const beforeClose = content.substring(0, closeIdx);
  const lastNewline = beforeClose.lastIndexOf('\n', closeIdx - 1);
  const lastLine = content.substring(lastNewline + 1, closeIdx).trim();

  // If last line doesn't end with comma, add one
  let insertPoint = closeIdx;
  if (!lastLine.endsWith(',')) {
    // Add comma after the last language value
    const lastQuote = content.lastIndexOf('"', closeIdx - 1);
    content = content.substring(0, lastQuote + 1) + ',' + content.substring(lastQuote + 1);
    insertPoint = lastQuote + 2; // position after ",\n"
  }
  ...
  content = content.substring(0, insertPoint) + newLangs + '\n    ' + content.substring(insertPoint);
  updatedCount++;
}

// Same for excerpts
for (const [slug, langMap] of Object.entries(excerptMap)) {
  const slugPattern = 'slug: "' + slug + '"';
  const slugIdx = content.indexOf(slugPattern);
  if (slugIdx === -1) continue;

  const excerptIdx = content.indexOf('excerpt: {', slugIdx);
  if (excerptIdx === -1 || excerptIdx > slugIdx + 500) continue;

  let searchFrom = excerptIdx;
  let closeIdx = -1;
  while (searchFrom < content.length) {
    const nextClose = content.indexOf('},', searchFrom);
    if (nextClose === -1) break;
  ...
  const blockContent = content.substring(excerptIdx, closeIdx);
  const existingLangs = allLangs.filter(l => blockContent.includes(l + ':'));
  const missing = allLangs.filter(l => !existingLangs.includes(l));
