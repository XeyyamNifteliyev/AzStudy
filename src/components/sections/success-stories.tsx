...
export async function SuccessStories({ locale }: SuccessStoriesProps) {
  ...
  const reviews = all
    .filter((r) =>
      seen.has(r.universityId) ? false : seen.add(r.universityId),
    )
    .slice(0, 6);
      ...
      <FadeIn className="container-page">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {reviews.map((review) => (
            <Card
              key={review.id}
              className="transition-shadow duration-300 ease-fluid hover:shadow-flat-hover"
            >
                ...
                <div className="flex items-center gap-3 border-t border-border pt-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      {lx(review.programStudied, locale)}
                    </p>
                  </div>
                </div>
              </CardContent>
